...
from kivy.core.window import Window #ssetting the window size in kivy 
...
class medCalcNight(RelativeLayout):#the medcalc nightmode class 
    ...
    def __init__(self,**kwargs):#creating the constructor 
        ...
        def createHomePage(self):
            ...
            btnSupply = Button(text="Supply per 1mL",size_hint=(0.4,0.1), pos_hint={'x':0.32, 'y':0.6})
            btnSupply.bind(on_press=Supply)#binding the button to a function 
            layoutHome.add_widget(btnSupply)
            
            btnIvp = Button(text="IV Push",size_hint=(0.4,0.1), pos_hint={'x':0.32, 'y':0.5})
            btnIvp.bind(on_press=IvPush)
            layoutHome.add_widget(btnIvp)
            
            btnIvd = Button(text="IV Drip Problem",size_hint=(0.4,0.1), pos_hint={'x':0.32, 'y':0.4} )
            btnIvd.bind(on_press=IvDrip)
            layoutHome.add_widget(btnIvd)
            
            btnDopa = Button(text="Dopamine Problem",size_hint=(0.4,0.1), pos_hint={'x':0.32, 'y':0.3})
            btnDopa.bind(on_press=Dopamine)
            layoutHome.add_widget(btnDopa)
            
            btnConvert = Button(text="Conversion",size_hint=(0.4,0.1), pos_hint={'x':0.32, 'y':0.2})
            btnConvert.bind(on_press=Conversion)
            layoutHome.add_widget(btnConvert)
            
            btnSettings = Button(text="Settings",size_hint=(0.4,0.1),pos_hint={'x':0.32, 'y':0.1})
        ...
        def Supply(self):
            ...
            lblMgs = Label(text="Enter milligrams\n                (MG): ",size_hint=(0.3,0.1), pos_hint={'x':0.15, 'y':0.6})
            layoutHome.add_widget(lblMgs)
            txtMgs = TextInput(size_hint=(0.3,0.1), pos_hint={'x':0.48, 'y':0.6}, multiline=False)
            layoutHome.add_widget(txtMgs)
            
            lblMls = Label(text="Enter milliliters\n                 (mL): ",size_hint=(0.3,0.1), pos_hint={'x':0.15, 'y':0.5})
            layoutHome.add_widget(lblMls)
            txtMls = TextInput(size_hint=(0.3,0.1), pos_hint={'x':0.48, 'y':0.5}, multiline=False)
            layoutHome.add_widget(txtMls)
            
            lblMgspMl = Label(text="      Milligrams \n         per 1 mL: ",size_hint=(0.3,0.1), pos_hint={'x':0.15, 'y':0.4})
            layoutHome.add_widget(lblMgspMl)
            
            txtAnswerSupp = TextInput(size_hint=(0.3,0.1), pos_hint={'x':0.48, 'y':0.4}, multiline=False, disabled=True) #disabled sets text input to read only 
            ...
            btnReturnHome = Button(text="Return to Home",size_hint=(0.35,0.1), pos_hint={'x':0.18, 'y':0.3})
            btnReturnHome.bind(on_press=returnHome)
            layoutHome.add_widget(btnReturnHome)
            
            btnCalculate = Button(text="Calculate",size_hint=(0.3,0.1), pos_hint={'x':0.52, 'y':0.3})
